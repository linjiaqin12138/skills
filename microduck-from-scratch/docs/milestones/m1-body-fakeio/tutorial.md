...
- **差异点**：① **坐标系是躯干坐标系，不是世界坐标系**——gravity 表达的是"重力在机器人自己视角里的方向"：直立时重力指向脚底，即 `[0, 0, -1]`（`src/io.rs:36-44`，机体系 z 向下为正）。机器人前倾时这个向量跟着转，所以它对"我歪了多少"是最直接的信号。② `quat`（四元数，wxyz 序，`src/io.rs:31-32`）是 gyro/accel 融合后的完整姿态，数学上没有万向锁但没有直觉——M2 的观测向量只用 gyro+gravity 这 6 维，quat 留给 M5 跌倒检测。③ 本章的 FakeIo 恒定返回直立姿态（`ImuData::default()`），不做滤波不建模——占位而已，真的 IMU 数据从 M4 的仿真器来。
...
3. **插值锚进程启动 = 总线恢复时姿态跳变**。如果插值进度从进程启动起算，舵机电源晚就绪 3 秒，总线恢复那一刻插值已走完，写入值从"躺平"直接跳到 home——真机 = 舵机猛抽。锚定第一次 read 成功（`src/control.rs:59-62`）后，这个失败模式在结构上不存在。教训的一般形：**涉及物理设备的进度条，起点必须锚在"设备第一次给出真实反馈"的时刻**。
...
完整背景卡片在本章概念铺垫的【IMU】一节，这里给压缩版：IMU 是躯干上的芯片，回答两件事——"转多快"（gyro，三维角速度，类比 `DeviceMotionEvent.rotationRate`）和"重力相对我指向哪"（gravity，三维单位向量，类比 `accelerationIncludingGravity` 归一化）。最容易错的一点：gravity 用的是**躯干坐标系**——重力在机器人自己视角里的方向，直立时 = `[0, 0, -1]`（指向脚底），机器人一歪这个向量就跟着转，所以它直接编码"我歪了多少"。quat 是融合后的完整姿态四元数，没万向锁但没直觉。M2 的观测向量只用 gyro+gravity 前 6 维，quat 留给 M5 跌倒检测——所以本章 FakeIo 恒定返回直立姿态占位就够了。

...
### 常见疑问：借用、解引用、`as`、生命周期和编译期执行

- `Some` / `Ok` / `None` / `Err` 不是宏，也不是普通函数；它们是枚举变体构造器：`Some(5)`、`Ok("x")` 分别构造 `Option<i32>` 和 `Result<&str, E>` 的值。`match` 里用的 `Some(Ok(line))` 是“模式”，不是比较语句。
- `&T` 是借用，`*r` 是解引用。`let r = &x;` 只拿到 `x` 的引用，不拿走 `x` 的所有权；`println!("{}", *r)` 才是打印引用指向的真实值。`println!("{:p}", r)` 打印地址，`println!("{}", r)` 不能直接按值输出，除非这个引用类型本身实现了 `Display` 的特殊语义，否则编译不过。
- `as f64` 是显式数值转换：`out[slot] = *value as f64;` 表示“把 `*value` 从原类型强制转换成 `f64`，再存进输出数组”。如果左右类型不一致，Rust 默认不自动隐式转换，需要你写 `as`。
- 函数可以在编译期执行，通常是 `const fn`。它不是等程序运行时才算，而是编译器在编译阶段把常量表达式提前算好。`const fn add(a: i32, b: i32) -> i32 { a + b }` 可用于 `const N: i32 = add(2, 3);`，最终 `N` 就是 5。普通 `fn` 只有在运行时才执行。
- 生命周期和“栈/堆”不是同一层概念：栈/堆描述数据放在哪；生命周期描述引用在什么时间段合法。Rust 的借用检查保证“引用不能活得比它指向的数据更久”。例如 `fn bad() -> &String { let s = String::from("hi"); &s }` 会失败，因为 `s` 在函数结束后被销毁，返回引用就成了悬垂引用。
- `match frame { Some(Ok(line)) => line, _ => break, }` 的意思是：若 `frame` 是 `Some(Ok(value))`，就把里面的 `value` 绑定为 `line`；其他情况直接 `break`。这就是典型的“模式匹配解构”写法，和 `if` 的“布尔条件判断”完全不同。
